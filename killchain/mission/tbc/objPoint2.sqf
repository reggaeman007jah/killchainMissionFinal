...
sleep 5;
// enemy vics 
_posAnchor = RGG_PatrolPoints select 2;
_pos = [_posAnchor, 0, 40] call BIS_fnc_findSafePos;
_opforVic = [_pos, 180, "O_G_Offroad_01_armed_F", east] call BIS_fnc_spawnVehicle;
_opforVic doMove _initStartPos;
systemChat "delete when tested - technical created";
...
while {RFCHECK2} do {
	...
	if (_cycles >= 5) then {
		// progress regardless 
		RFCHECK2 = false;
		systemChat "_cycles >= 5 / cyclepush here";
	} else {
		_cycles = _cycles + 1;
		...
		// add an aggressive move here - take some opfor and send to older point 
		if ((_coreOpfor >= 10) && (_redzoneIndi <= 5)) then {
			// get half and send to older point  
			...
			// now ID larger groups 
			{
				if (count (units _x) < 3) then { _aggressorGroups pushBack _x };
			} forEach _opforGroups;

			// count aggressors 
			_arrgressorCount = count _aggressorGroups;

			// aggressive move order 
			_cnt = count _arrgressorCount;
			if (_cnt >1) then {
				_aggressorGroup = _aggressorGroups select 0;
				_aggressorGroup move _initStartPos;
				systemChat "Delete when tested - you should be seeing an aggressor group move to previously taken patrol point";
			};
		};
...
if (_cnt >1) then {
	...
	[west, _removeBluSpawn] call BIS_fnc_removeRespawnPosition;
	[independent, _removeIndSpawn] call BIS_fnc_removeRespawnPosition;
...
// init Point 1 Obj  
[_anchor, _objPos] execVM "killChain\mission\objPoint3.sqf";
